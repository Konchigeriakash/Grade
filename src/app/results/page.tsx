...
function ResultsComponent() {
  ...
  React.useEffect(() => {
    if (finalResult && previousCgpa && previousCredits) {
      const prevCgpaNum = parseFloat(previousCgpa);
      const prevCreditsNum = parseFloat(previousCredits);
      const currentCredits = finalResult.results.reduce(
        (sum, r) => sum + r.credits,
        0
      );
      ...
      if (
        ...
        const cumulativeCgpa =
          (prevCgpaNum * prevCreditsNum + finalResult.sgpa * currentCredits) /
          (prevCreditsNum + currentCredits);
        setOverallCgpa(parseFloat(cumulativeCgpa.toFixed(2)));
      } else {
        setOverallCgpa(null);
      }
    } else {
  ...
  }, [previousCgpa, previousCredits, finalResult]);
  ...
  const handleEditGrade = () => {
    ...
    if (subjectToUpdate && newGradeInfo) {
      ...
      subjectToUpdate.requiredSeeMarks = calculateRequiredSeeMarks(
        newGradeInfo.marks,
        subjectToUpdate.cie
      );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="text-center bg-muted p-6 rounded-lg flex flex-col justify-center">
                    ...
                    {overallCgpa !== null ? (
                       ...
                       <p className="text-3xl text-muted-foreground/60 flex-grow flex items-center justify-center">Enter previous CGPA and credits</p>
                    )}
                </div>
            </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="text"
                    inputMode="decimal"
                    placeholder="Total credits before this semester"
                    value={previousCredits}
                    onChange={(e) => setPreviousCredits(e.target.value)}
                  />
                </div>
                <p className="col-span-1 md:col-span-2 text-xs text-muted-foreground">
                    Enter your previous CGPA and prior credit total to calculate the accurate cumulative CGPA using credit-weighted averaging.
                </p>
            </div>
